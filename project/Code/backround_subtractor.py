import cv2
import numpy as np
from Code.utils import extract_video_parameters, apply_mask_on_color_frame, \
    convert_frames_color
...
def subtruct_background(video_frames: list, config: dict):
    print("Starting Background Subtraction...")

    video_frames_bgr = video_frames
    video_frames_hsv = convert_frames_color(video_frames, color_space=cv2.COLOR_BGR2HSV)
    n_frames = len(video_frames_bgr)

    backSub = cv2.createBackgroundSubtractorKNN()
    mask_list = np.zeros((n_frames, config['video_params']['h'], config['video_params']['w'])).astype(np.uint8)
    masks = np.zeros((n_frames, config['video_params']['h'], config['video_params']['w'])).astype(np.uint8)
    after_mask = np.zeros((n_frames, config['video_params']['h'], config['video_params']['w'], 3)).astype(np.uint8)
